...
%s    container_name: %s
    ...
    volumes:
      - /var/www:/var/www
      - ./conf.d/99-serverdash.ini:/usr/local/etc/php/conf.d/99-serverdash.ini:ro
      - ./php-fpm.d/zz-serverdash.conf:/usr/local/etc/php-fpm.d/zz-serverdash.conf:ro
    deploy:
      resources:
        limits:
          memory: %s
    environment:
      - TZ=%s
    labels:
      serverdash.plugin: php
      serverdash.runtime: "%s"
...
func GeneratePHPIni(cfg PHPIniConfig) (string, error) {
	...
	var lines []string
	lines = append(lines, "; Generated by ServerDash PHP Manager")
	lines = append(lines, "")
