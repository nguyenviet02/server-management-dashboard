#!/usr/bin/env bash
# ============================================================================
#  ServerDash — CLI Management Tool
#  https://web.casa
#
#  Usage:
#    serverdash                         Interactive menu
#    serverdash panel <command>         Panel management
#    serverdash caddy <command>         Caddy management
#    serverdash info                    System information
#    serverdash update                  Update ServerDash
#    serverdash reset-password          Reset admin password
#    serverdash version                 Show version
#    serverdash help                    Show help
# ============================================================================
...
SERVER_BIN="/usr/local/bin/serverdash-server"
CADDY_BIN="/usr/local/bin/caddy"
ENV_FILE="/etc/serverdash/serverdash.env"
DATA_DIR="/var/lib/serverdash"
LOG_DIR="/var/log/serverdash"
SERVICE_NAME="serverdash"
...
# ==================== Environment ====================
load_env() {
    SERVERDASH_PORT="${SERVERDASH_PORT:-39921}"
    if [[ -f "$ENV_FILE" ]]; then
...
cmd_panel() {
    ...
    case "$action" in
        ...
        *)       error "Unknown panel command: $action"; echo "Usage: serverdash panel {status|start|stop|restart|logs}"; exit 1 ;;
    esac
}
...
cmd_panel_status() {
    load_env
    echo ""
    echo -e "${BOLD}ServerDash Panel Status${NC}"
    echo "────────────────────────────────"
    ...
    echo -e "  Port:    ${SERVERDASH_PORT}"
    echo -e "  URL:     http://$(get_server_ip):${SERVERDASH_PORT}"
...
cmd_panel_start() {
    ...
    info "Starting ServerDash panel..."
...
cmd_panel_stop() {
    ...
    info "Stopping ServerDash panel..."
    systemctl stop "$SERVICE_NAME"
    success "Panel stopped"
}

cmd_panel_restart() {
    info "Restarting ServerDash panel..."
...
cmd_caddy() {
    ...
    case "$action" in
        ...
        *)       error "Unknown caddy command: $action"; echo "Usage: serverdash caddy {status|start|stop|restart|upgrade|logs}"; exit 1 ;;
    esac
}
...
cmd_info() {
    ...
    echo ""
    echo -e "${BOLD}═══════════════════════════════════════════${NC}"
    echo -e "${BOLD}  ServerDash System Information${NC}"
    echo -e "${BOLD}═══════════════════════════════════════════${NC}"
    ...
    echo "  ─────────────────────────────────"
    echo -e "  ServerDash:   $(get_serverdash_version)"
    ...
    echo "  ─────────────────────────────────"
    echo -e "  Panel Port: ${SERVERDASH_PORT}"
    echo -e "  Panel URL:  http://$(get_server_ip):${SERVERDASH_PORT}"
...
cmd_update() {
    ...
    local current_ver
    current_ver=$(get_serverdash_version)

    if [[ "$current_ver" == "$latest_ver" ]]; then
        info "ServerDash is already at the latest version (v${latest_ver})"
        return 0
    fi
    ...
    local url="https://github.com/${GITHUB_REPO}/releases/download/v${latest_ver}/serverdash_${latest_ver}_linux_${arch}.tar.gz"

    local tmpdir
    tmpdir=$(mktemp -d)
    trap 'rm -rf "$tmpdir"' EXIT

    info "Downloading ServerDash v${latest_ver}..."
    if ! curl -fsSL "$url" -o "${tmpdir}/serverdash.tar.gz"; then
        error "Download failed"
        exit 1
    fi

    tar -xzf "${tmpdir}/serverdash.tar.gz" -C "$tmpdir"

    # Find the binary in the extracted files
    local new_bin="${tmpdir}/serverdash"
    if [[ ! -f "$new_bin" ]]; then
        new_bin="${tmpdir}/serverdash-server"
    fi
    if [[ ! -f "$new_bin" ]]; then
    ...
    if panel_is_running; then
        rm -f "${SERVER_BIN}.bak"
        success "ServerDash updated to v${latest_ver}"
    else
...
cmd_reset_password() {
    if [[ ! -x "$SERVER_BIN" ]]; then
        error "ServerDash server binary not found: $SERVER_BIN"
        exit 1
    fi
...
cmd_version() {
    echo "ServerDash CLI v${VERSION}"
    if [[ -x "$SERVER_BIN" ]]; then
        echo "ServerDash Server $($SERVER_BIN --version 2>/dev/null || echo 'v?')"
    fi
    if command -v "$CADDY_BIN" &>/dev/null; then
...
cmd_help() {
    cat <<'HELP'
ServerDash — Server Management CLI

Usage:
  serverdash                           Interactive menu
  serverdash <command> [options]        Direct command

Panel Management:
  serverdash panel status              Show panel status
  serverdash panel start               Start the panel
  serverdash panel stop                Stop the panel
  serverdash panel restart             Restart the panel
  serverdash panel logs [-f]           View panel logs (-f to follow)

Caddy Management:
  serverdash caddy status              Show Caddy status
  serverdash caddy start               Start Caddy
  serverdash caddy stop                Stop Caddy
  serverdash caddy restart             Restart Caddy
  serverdash caddy upgrade [version]   Upgrade Caddy (latest if no version)
  serverdash caddy logs [-f]           View Caddy logs (-f to follow)

Other:
  serverdash info                      Show system information
  serverdash update                    Update ServerDash to latest version
  serverdash reset-password            Reset admin password
  serverdash version                   Show version information
  serverdash help                      Show this help message
HELP
}
...
interactive_menu() {
    ...
    while true; do
        ...
        echo ""
        echo -e "${BOLD}══════════════════════════════════════════════${NC}"
        echo -e "${BOLD}  ${GREEN}ServerDash${NC}${BOLD} v$(get_serverdash_version)${NC}"
        echo -e "${BOLD}══════════════════════════════════════════════${NC}"
        echo ""
        ...
        echo -e "  ${BOLD}2)${NC} Stop Panel             ${BOLD}7)${NC} Update ServerDash"
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        error "This command requires root privileges. Use: sudo serverdash $*"
        exit 1
    fi
}
